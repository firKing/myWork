...
//            console.log(links);
            links.each(function(){
                $(this).removeClass("now").css({"background-color":"transparent"});
            });
            $(links[num]).addClass("now").css({"background-color":"#fff"});
            //隐藏说明栏
...
//            console.log('numsssss:'+num);
        ...
        var links=imgs.parent().find('li');
        $(links[0]).css({"background-color":"#fff"});
        //每个link加上序号
        for(var j=0;j<len;j++){
            $(links[j]).attr({'n':j});
        }
        ...
        button_left.bind("click",function(){
            if(changeStart)clearInterval(changeStart);
            var n = parseInt($(".now").attr('n'))-2;
            var now = n == -2 ? 3 : n;
            num = changeImg.change(imgs,imgs_left,now)();
//            console.log(now);
            changeStart=setInterval(moveStart,4000);
        });
        button_right.bind("click",function(){
            if(changeStart)clearInterval(changeStart);
            var n = parseInt($(".now").attr('n'));
            var now = n == 4 ? -1 : n;
            num = changeImg.change(imgs,imgs_left,now)();
//            console.log(now);
            changeStart=setInterval(moveStart,4000);
